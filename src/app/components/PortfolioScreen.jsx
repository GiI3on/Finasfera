...
/* === komponenty z tego samego katalogu === */
...
export default function PortfolioScreen({ title = "Mój Portfel" }) {
  ...
  const [fireGoal, setFireGoal] = useState(0);

  const { value: allPortfoliosValue } = useAllPortfoliosPLN(user?.uid);
  ...
  useEffect(() => {
    ...
    try {
      const keysToTry = ["fireCalculator:lastPlan", "fire:lastPlan", "fire:calc", "calculator:fire"];
      for (const key of keysToTry) {
        const raw = localStorage.getItem(key);
        if (raw) {
          const parsed = JSON.parse(raw);
          const exp = Number(parsed?.annualExpenses ?? parsed?.expenses ?? 0);
          const mult = Number(parsed?.targetMultiplier ?? parsed?.mult ?? 25);
          if (exp > 0 && mult > 0) {
            setFireGoal(exp * mult);
            break;
          }
        }
      }
  ...
  useEffect(() => {
    ...
    try {
      const pid = pidNorm === "" ? null : pidNorm; 
  ...
  // PANCERNA MATEMATYKA FIRE
  const safeAllPortfolios = Number.isFinite(allPortfoliosValue) ? allPortfoliosValue : 0;
  const safeTotalsCur = Number.isFinite(totals?.cur) ? totals.cur : 0;
  const currentTotalForFire = safeAllPortfolios > 0 ? safeAllPortfolios : safeTotalsCur;
  
  let firePercent = 0;
  if (fireGoal > 0) {
    const calc = Math.floor((currentTotalForFire / fireGoal) * 100);
    firePercent = Number.isFinite(calc) ? Math.min(100, Math.max(0, calc)) : 0;
  }
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 pb-24">
      ...
      {/* KPI — NA GÓRZE Z POPRAWIONYMI PROPORCJAMI */}
      <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        
        {/* KAFEL: Cel FIRE */}
        <div className="card flex flex-col h-full bg-gradient-to-br from-zinc-900 to-zinc-950 border-yellow-500/30 border shadow-lg">
          <div className="card-inner flex flex-col justify-between items-center h-full py-5">
            <h3 className="text-xs font-bold text-yellow-500 uppercase tracking-widest mb-3">Twój Cel FIRE</h3>
            {fireGoal > 0 ? (
              <>
                <div className="my-1">
                  <FireProgress percent={firePercent} />
                </div>
                <div className="text-xs text-zinc-400 mt-4 bg-black/40 px-4 py-1.5 rounded-full border border-zinc-800">
                  Brakuje: <span className="text-zinc-200 font-medium">{fmtPLN(Math.max(0, fireGoal - currentTotalForFire))}</span>
                </div>
              </>
            ) : (
              <div className="text-sm text-zinc-400 px-4 my-auto text-center">
                Nie ustawiłeś celu. <br/>
                <a href="/" className="text-yellow-400 underline mt-1 inline-block">Ustaw Kalkulator</a>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="card h-full">
          <div className="card-inner flex flex-col justify-center h-full py-5">
            <div className="text-sm text-zinc-400 font-medium mb-2">Wartość portfela</div>
            <div className="text-3xl lg:text-4xl font-bold tabular-nums text-zinc-100">
              {fmtPLN(totals.cur)}
            </div>
          </div>
        </div>
        ...
        <div className="card h-full">
          <div className="card-inner flex flex-col justify-center h-full py-5">
            <div className="text-sm text-zinc-400 font-medium mb-2">Dzienny zysk</div>
            <div className={`text-3xl lg:text-4xl font-bold tabular-nums ${totals.day >= 0 ? "text-emerald-400" : "text-red-400"}`}>
              {totals.day >= 0 ? "+" : ""}{fmtPLN(totals.day)}
            </div>
          </div>
        </div>

        {/* KAFEL: Całkowity zysk */}
        <div className="card h-full">
          <div className="card-inner flex flex-col justify-center h-full py-5">
            <div className="text-sm text-zinc-400 font-medium mb-2">Całkowity zysk</div>
            <div className={`text-3xl lg:text-4xl font-bold tabular-nums ${totals.gainAbs >= 0 ? "text-emerald-400" : "text-red-400"}`}>
              {totals.gainAbs >= 0 ? "+" : ""}{fmtPLN(totals.gainAbs)}
            </div>
            <div className="mt-2">
              <span className={`inline-flex px-2 py-0.5 rounded text-xs font-bold tabular-nums ${totals.gainAbs >= 0 ? "bg-emerald-400/10 text-emerald-400" : "bg-red-400/10 text-red-400"}`}>
                {totals.gainAbs >= 0 ? "▲" : "▼"} {fmtPct(totals.gainPct)}
              </span>
            </div>
          </div>
        </div>
